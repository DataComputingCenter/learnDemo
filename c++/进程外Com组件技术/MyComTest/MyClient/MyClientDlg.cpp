...
void CMyClientDlg::OnBnClickedOk()
{
	...
	CLSID  clsid; 
	hr=CLSIDFromProgID(OLESTR("MyComTest.Test"),&clsid);
	if(FAILED(hr))
	{
	...
	ITest  *ptr = NULL;
	hr=CoCreateInstance(clsid,NULL,CLSCTX_LOCAL_SERVER,__uuidof(ITest),(LPVOID*)&ptr);
	LONG rst = ptr->AddNum(1,2);
	ptr->Release(); // 释放实例
	CoUninitialize();
	CString strRst;
	strRst.Format(_T("%ld"),rst);
	AfxMessageBox(strRst);
